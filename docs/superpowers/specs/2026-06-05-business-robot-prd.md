...
| 指标 | 基线口径 | 目标值 | 度量方式 | 统计周期 |
| --- | --- | --- | --- | --- |
| 历史项目知识查询自助解决率 | 人工询问产品或研发后解决的历史项目查询量 | 上线 1 个月达到 60%，上线 3 个月达到 75% | 已由机器人直接回答且业务人员未转人工追问的查询数 / 历史查询总数 | 周、月 |
| 历史项目变更材料完整率 | 变更材料中同时包含原方案、目标方案、影响范围的比例 | 上线 1 个月达到 70%，上线 3 个月达到 85% | 满足必需字段的变更单数 / 变更单总数 | 周、月 |
| 产品经理重复澄清次数 | 产品经理针对同一需求向业务人员补问的平均次数 | 较基线下降 30% | 需求流转后产品补问次数 / 需求数 | 月 |
| 系统问题定位材料完整率 | 问题反馈中包含页面、路径、期望结果、实际结果、证据、复现步骤的比例 | 上线 1 个月达到 65%，上线 3 个月达到 80% | 满足问题定位必需字段的问题单数 / 问题单总数 | 周、月 |
| 意图识别人工纠正率 | 用户或运营手动切换场景的比例 | 上线 1 个月低于 15%，上线 3 个月低于 8% | 被人工纠正场景的会话数 / 进入场景路由的会话数 | 周、月 |
| 下游接受率 | 产品、研发、测试未退回即可进入处理的输出比例 | 上线 1 个月达到 70%，上线 3 个月达到 85% | 未被退回补充的输出数 / 已提交输出总数 | 周、月 |
| 输出可追溯率 | 输出中关键结论可追溯到原话、附件或知识来源的比例 | 100% | 抽样检查关键字段是否存在来源引用 | 周 |
...
- 知识冲突时不能直接合并为确定结论。
- 知识过期时应提示更新时间。
- 同一项目存在多份知识时，优先使用标记为“有效”的唯一生效版本。
- 如果没有唯一生效版本，则按知识状态进入冲突处理，不允许由模型自行判断哪份正确。
...
- 管理员可以标记冲突或过期知识。
- 同一项目同一知识类型只能有一条唯一生效版本；多条有效记录必须进入冲突状态。
