...
>
> 本版（V1.6）在 V1.5 基础上补齐评审中标记的 🟡 级缺口（审核 SLA、退回轮次、跨入口关联、多事项拆分、多模态配额、输出模板、埋点口径、可访问性等），逐项改动见文末「§33 修订记录（V1.5 → V1.6）」。凡涉及具体数值/SLA 处均标注「建议值（待评审确认）」。
...
| 图片/截图识别 | ≤ 8 秒 | 单张图片 |
| 视频/录屏处理 | 异步，按时长分档（建议值，待评审确认）：≤1 分钟 ≤60 秒；1–3 分钟 ≤180 秒；3–5 分钟 ≤300 秒 | 宽口径(500MB/5分钟/1080p)下的建议处理时延，完成后通知用户 |
...
为控制 ASR/OCR/VLM（尤其是视频）处理的算力成本，设置配额与预算护栏（以下数值均为建议值，待评审确认）：

- 单用户每日视频/录屏处理配额：建议 ≤ 20 分钟总时长或 ≤ 10 个任务，超出后任务进入低优先级队列并提示用户。
- 单用户每日图片识别配额：建议 ≤ 200 张，超出后限流提示。
- 全局多模态算力按月度预算管理，达到预算阈值（建议 80%）时触发运营告警；达到 100% 时新视频任务排队或降级为「仅存储、暂不解析」。
...
- 同一用户在不同入口发起的会话应能关联到统一会话记录。关联机制（建议方案，待评审确认）：以 SSO 统一用户标识为主键，结合项目/平台上下文与时间窗聚合；同一用户在不同入口针对同一事项的输入可合并为一个会话，或以「关联会话」形式互相引用，具体合并粒度由产品在设计阶段确定。
- 支持上传附件。
- 支持语音录入和语音转写。
- 支持查看机器人生成的结构化输出。
...
非结构化资料抽取出的精准字段候选需人工审核后才能晋升为结构化事实，审核时效直接影响知识查询解决率，需设 SLA 与积压护栏（以下为建议值，待评审确认）：

- 普通精准字段候选：自进入待审核队列起，建议 2 个工作日内完成审核。
- 高优先级候选（活跃项目费率、接口编号、是否见费出单等关键字段）：建议 4 小时内完成审核。
- 审核积压超过阈值（建议单项目待审核候选 > 50 条，或最早候选等待 > 3 个工作日）时，触发运营告警并提示扩充审核人力。
...
- 退回-补充设最大循环次数（建议值，待评审确认）：同一输出累计退回不超过 3 次；达到上限仍未达成一致时，应升级为人工协同（由产品经理或运营管理员介入，或转线下当面澄清），不再无限循环退回。
...
为支撑 §4 成功指标，需统一埋点口径（事件清单为建议值，待评审确认）：
...
- ASR 需支持普通话及保险领域热词；对常见方言与口音的支持程度作为建议优化项（待评审确认是否纳入首期）。
