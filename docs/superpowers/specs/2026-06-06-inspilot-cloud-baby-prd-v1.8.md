...
>
> 本版（V1.8）根据业务场景调整部署与集成口径：**默认公有云 SaaS、公网可访问**（面向经常出差的市场人员）；**工单系统对接钉钉工单流程**；**多模态优先公有云**以节约成本、适配小微企业。逐项见文末「§35」。
...
- **流转集成层**：对接需求管理系统与**钉钉工单流程**（系统问题/优化反馈走钉钉工单），支持提交、受理、退回补充状态回传。
...
| 下游接受率 | 产品、研发、测试明确受理并进入处理的输出比例 | 上线 1 个月达到 70%，上线 3 个月达到 85% | 被下游标记“已受理”或成功建单的输出数 / 已提交输出总数 | 系统埋点、需求系统或钉钉工单状态回传；无正向受理动作的不计入分子 | 周、月 |
...
- 标准 JSON 需要按输出类型提供稳定 Schema，便于需求管理系统、钉钉工单或数据平台解析和建单。
...
| 下游系统（需求系统/钉钉工单）不可用 | 结构化输出正常生成，标记为"待提交"，下游恢复后自动或手动重试 | 提示"输出已生成，下游系统暂时无法提交，恢复后自动重试" |
...
- 需求管理系统与**钉钉工单流程**需要支持接收标准 JSON、附件证据、输出版本和退回状态，并能回传受理/退回状态。
