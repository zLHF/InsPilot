...
> V1.5 基于 V1.4 评审意见修订：解决 5 处硬冲突（视频规格、PII 留存、角色体系、精准字段准确率口径、冷启动覆盖率口径）并回填全部子章节编号，详见 §32。
>
> V1.6 在 V1.5 基础上补齐评审中标记的 🟡 级缺口（审核 SLA、退回轮次、跨入口关联、多事项拆分、多模态配额、输出模板、埋点口径、可访问性等），逐项改动见文末「§33 修订记录（V1.5 → V1.6）」。
>
> V1.7（定稿）将 V1.6 中标注「待评审确认」的全部建议值定稿为生效值（视频 SLA、多模态配额、算力预算告警、跨入口关联、审核 SLA、退回上限、埋点口径、方言范围），逐项决策见文末「§34」。
>
> V1.8 根据业务场景调整部署与集成口径：**默认公有云 SaaS、公网可访问**（面向经常出差的市场人员）；工单对接钉钉工单流程；**多模态优先公有云**以节约成本、适配小微企业。逐项见文末「§35」。
>
> 本版（V1.9）：新需求/变更需求也在钉钉发起，需求下游从独立需求管理系统改为**钉钉需求流程**，与钉钉工单流程共同构成**统一的钉钉下游通道**。逐项见文末「§36」。
