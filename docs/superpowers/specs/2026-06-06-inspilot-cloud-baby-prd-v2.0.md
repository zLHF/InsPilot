...
>
> 本版（V2.0）：新增 §32「RAG 知识库资料构成与达标说明」，系统化说明 RAG 资料范围、形成方式与达标要求，并补充平台/项目定位辅助字段；原修订记录顺延为 §33–§37。逐项见文末「§38」。
...
- 含 PII 的内容在传往公有云多模态服务前应尽量先脱敏/打码；确需原文识别的，限定授权云厂商并通过 DPA + 加密通道处理，识别结果回传后云端即删。
...
> 注：表中「运营管理员」即 §6.6 定义的运营管理员。系统管理员（§6.7）负责用户、角色、权限与系统配置，不直接参与业务会话与知识内容维护，故未列入上表，其权限见 §16。
...
- 用户输入中的客户姓名、身份证号、手机号等 PII 信息默认以脱敏形式（占位符）展示；原文加密存储，仅授权角色在权限范围内可查看（见 §19.3），留存期限跟随所属会话的附件留存策略（见 §8），不单独长期保留明文。
- 脱敏规则可配置，由安全负责人维护。
...
- 需要先定位项目。
- 项目不唯一时追问关键字段。
- 项目不唯一或主标识缺失时，可结合 §32.2 的平台/项目定位辅助字段（地区、最近出单情况、单证格式、加密铭文等）缩小候选范围。
- 回答应展示知识来源或来源摘要。
...
- 全量上线前完成全部在册项目的知识库导入，并达到 §13.6 的全量验收门槛（Top N 覆盖率 ≥ 80%、精准字段准确率 ≥ 98%）。
...
- 通过持续投喂历史项目案例与历史信息，由系统解析、学习、整合，逐步形成并丰富知识体系。
...
| 2 | 平台/项目定位辅助字段 | 补充地区、最近出单情况、单证格式、加密铭文等辅助定位字段，并在 FR-006 交叉引用 | §32.2、FR-006 |
